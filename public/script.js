...
function renderTypingBar(ctx,isDM,barId){
  ...
  const names=typers.length>2?`${typers.slice(0,-1).join(', ')} and ${typers[typers.length-1]}`:typers.join(' and ');
...
(async()=>{
  ...
  showSkeleton();
  try{
    const saved=localStorage.getItem('nebula_sess'); if(!saved){hideSkeleton();return;}
...
function buildMessage(m, idx, isFirst, ctxId, isDM){
  ...
  if(!m.deleted&&(canAct||currentUser)){
    ...
    if(currentUser){
      ...
      rb.addEventListener('click',e=>{e.stopPropagation();openEmoji(e,ctxId,idx,isDM);}); acts.appendChild(rb);
    }
    if(isOwn){
      const eb=document.createElement('button'); eb.className='mab'; eb.textContent='✎';
      eb.addEventListener('click',()=>startEdit(ctxId,idx,isDM)); acts.appendChild(eb);
    }
    if(canAct){
      const db2=document.createElement('button'); db2.className='mab d'; db2.textContent='✕';
      db2.addEventListener('click',()=>promptDel(ctxId,idx,isDM)); acts.appendChild(db2);
    }
...
function startEdit(ctxId,idx,isDM=false){
  ...
  save.addEventListener('click',doSave); can.addEventListener('click',doCancel);
  ...
  if(banned.length){
    ...
    banned.forEach(([u,a])=>{
      ...
      row.innerHTML=`<div class="adm-ava" style="background:${userColor(u)}">${avatarLetter(u)}</div>`
        +`<div class="adm-name" style="opacity:.5">${esc(u)}</div><div style="font-size:.63rem;color:var(--text-muted);margin-left:.3rem;opacity:.5;">${esc(a.name||'')}</div><span style="font-size:.63rem;color:var(--danger);">Banned</span>`;
