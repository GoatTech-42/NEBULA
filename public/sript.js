...
async function sendMessage(){
  ...
  try{
    ...
    const updated  = [...existing, { user:currentUser.username, text:text, time:tsNow(), reactions:{} }].slice(-MAX_CHANNEL_MSGS);
  ...
  const doSave = async () => {
    ...
    try{
      if(isDM){
        ...
        let ms   = [...(DB.dms[k]||[])]; ms[idx] = { ...ms[idx], text:nv, edited:true };
        const nd = { ...DB.dms, [k]:ms }; DB.dms = nd; scheduleDMWrite(nd);
      } else {
        let ms   = [...(DB.messages[ctxId]||[])]; ms[idx] = { ...ms[idx], text:nv, edited:true };
...
async function sendDM(){
  ...
  try{
    ...
    const updated  = [...existing, { user:currentUser.username, text:text, time:tsNow(), reactions:{} }].slice(-MAX_DM_MSGS);
...
/* ── openZone — loads game into static #game-frame iframe ── */
function openZone(z){
...
function toggleFS(){
  const frame = document.getElementById('game-frame'); if(!frame) return;
